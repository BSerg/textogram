...
def get_article_views_today(article_id, date=timezone.now(), tz_name='Europe/Moscow'):
    ...
    auth_user_fingerprints = ArticleView.objects\
        .filter(article_id=article_id, user__isnull=False)\
        .values_list('fingerprint', flat=True)

    views = ArticleView.objects.filter(article_id=article_id, created_at__gte=today_start) \
        .exclude(user__isnull=True, fingerprint__in=auth_user_fingerprints)
    return time.mktime(today_start.timetuple()), views.distinct('fingerprint').count()
...
def get_article_views_month(article_id, tz_name='Europe/Moscow'):
    ...
    views = ArticleView.objects.filter(article_id=article_id, created_at__gte=month_start, created_at__lt=today_start) \
        .exclude(user__isnull=True, fingerprint__in=auth_user_fingerprints)
    return time.mktime(month_start.timetuple()), views.distinct('fingerprint').count()
...
def get_article_views_prev_month(article_id, tz_name='Europe/Moscow'):
    ...
    views = ArticleView.objects\
        .filter(article_id=article_id, created_at__gte=prev_month_start, created_at__lt=month_start) \
        .exclude(user__isnull=True, fingerprint__in=auth_user_fingerprints)
    return time.mktime(prev_month_start.timetuple()), views.distinct('fingerprint').count()
...
def get_article_views_total(article_id, tz_name='Europe/Moscow'):
    ...
    views = ArticleView.objects\
        .filter(article_id=article_id, created_at__lt=today_start) \
        .exclude(user__isnull=True, fingerprint__in=auth_user_fingerprints)
    return time.mktime(today_start.timetuple()), views.distinct('fingerprint').count()
